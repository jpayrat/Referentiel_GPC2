...
// 1 - Récupération des valeurs du formulaire (ilot) et requête AJAX
	...
	function recup_form_ilot()
	{
		...
		if( $('#ilot_tape').val() == '')
		{
			if($('#ilot_list').val() == '***') { ilot = '' }
			else { ilot = $('#ilot_list').val().substr(0, 3);}
		}
		else { ilot = $('#ilot_tape').val(); }
	...
	{
		data_get = '';
		if(critere == 'select_all') { data_get = 'select_all=ok&iloCodeBase='+iloCodeBase+'&Complement_Titre='+Complement_Titre; }
		else if(critere == 'select_one') { data_get = 'select_one=ok&iloCodeBase='+iloCodeBase+'&Complement_Titre='+Complement_Titre+'&ilot='+ilot; }
		else { data_get = 'rechercheGlobal='+rechercheGlobal+'&optim='+optim+'&ilot='+ilot+'&typeIlot='+typeIlot+'&used='+used+'&competence='+competence+'&serviceCible='+serviceCible+'&entreprise='+entreprise+'&siteGeo='+siteGeo+'&domaineAct='+domaineAct+'&iloCodeBase='+iloCodeBase+'&Complement_Titre='+Complement_Titre; }
	...
	//	alert(data_get);
	      ...
	      // on envoie la valeur recherché en GET au fichier de traitement
			$.ajax({
				...
				url : '_controleurs/'+php_ilot , // url du fichier de traitement
				data : data_get,
	...
	$('#reinit_form').on('click', function(){ $('#form_ilot')[0].reset(); recup_form_ilot(); requete_formulaire(); });
	$('#all_form').on('click', function(){ $('#form_ilot')[0].reset(); requete_formulaire('select_all');  });

	$('#rechercheGlobal').on('keyup', function(){ recup_form_ilot(); requete_formulaire(); });
	$('input:radio[name="optim"]').change(function() { recup_form_ilot(); requete_formulaire(); });
    $('#ilot_list').on('change', function() { $('#ilot_tape').val(''); // réinit de l'autre choix de l'îlot (ilot tapé)
        recup_form_ilot(); requete_formulaire(); });
    $('#ilot_tape').on('keyup', function() { $('#ilot_list').prop('selectedIndex',0);// réinit de l'autre choix de l'îlot (ilot listé)
        recup_form_ilot(); requete_formulaire(); });
    $('#typeIlot').on('change', function() { recup_form_ilot(); requete_formulaire(); });
	$('#used').on('change', function() { recup_form_ilot(); requete_formulaire(); });
	$('#competence').on('change', function() { recup_form_ilot(); requete_formulaire(); });
    $('#serviceCible').on('change', function() { recup_form_ilot(); requete_formulaire(); });
    $('#entreprise').on('change', function() { recup_form_ilot(); requete_formulaire(); });
    $('#siteGeo').on('change', function() { recup_form_ilot(); requete_formulaire(); });
    $('#domaineAct').on('change', function() { recup_form_ilot(); requete_formulaire(); });
...
// Affichage des infosBulles
	...
	}, '.champ').mousemove(function(event) { $('.infoBulle').css('top', (event.pageY - 10) + 'px').css('left', (event.pageX + 20) + 'px'); });
	...
	$('#results_ilot').on('click', '#majBDD_tm_ilots', function (){
		if (confirm('Etes-vous sûr de voulour mettre à jour cet îlot ?')) {
			...
			$.ajax({
				...
				data : data_get,
				beforeSend : function() { $('#results_ilot').empty(); }, // traitements JS à faire AVANT l'envoi
				success : function(data){
				// traitements JS à faire APRES le retour du fichier .php
